...
def _session_con_busqueda(usuario_id: int | None) -> AsyncMock:
    ...
    resultado.scalars.return_value.all.return_value = []  # sin objetos equipados
...
async def test_incrementa_xp_y_registra_evento_cuando_el_nombre_matchea():
    ...
    assert session.execute.await_count == 3  # búsqueda por nombre + bono de objetos + UPDATE atómico
...
@pytest.mark.asyncio
async def test_bono_de_xp_aumenta_el_xp_acreditado(monkeypatch):
    from app.services.tienda import BonoStats

    session = _session_con_busqueda(usuario_id=7)
    monkeypatch.setattr("app.services.xp.tienda.bono_de_usuario", AsyncMock(return_value=BonoStats(xp_pct=10)))

    await otorgar_xp(session, "Juan", 20, "prueba")

    evento = session.add.call_args.args[0]
    assert evento.cantidad == 22
...
async def test_usuario_id_directo_evita_buscar_por_nombre():
    ...
    assert session.execute.await_count == 2  # bono de objetos + UPDATE, sin búsqueda por nombre
    evento = session.add.call_args.args[0]
...
async def test_sin_usuario_id_directo_cae_a_busqueda_por_nombre():
    ...
    assert session.execute.await_count == 3  # búsqueda por nombre + bono de objetos + UPDATE
    evento = session.add.call_args.args[0]
    assert evento.usuario_id == 7
